...
bool StreamReassembler::write_substring(
    ByteStream &output, 
...
void StreamReassembler::add_and_merge_unassembled_bytes(
  map<size_t, string> &segments, 
...
void StreamReassembler::push_substring(
    const string &data, 
    const size_t index, 
    const bool eof
) {
  ...
  while (it != _segments.end()) {
    if (it->first <= _next_unassembled && 
        it->first + it->second.size() > _next_unassembled) {

...
//! \param[in] retx_timeout the initial amount of time to wait before retransmitting 
//! the oldest outstanding segment
//! \param[in] fixed_isn the Initial Sequence Number to use, if set (otherwise uses a 
//! random ISN)
TCPSender::TCPSender(
...
//! \returns `false` if the ackno appears invalid (acknowledges something the TCPSender 
//!  hasn't sent yet)
bool TCPSender::ack_received(const WrappingInt32 ackno, const uint16_t window_size) {
  ...
  if (absolute_ackno >= _acked) {
    ...
    while(!_segments_in_flight.empty()) {
      uint64_t seg_seqno = unwrap(
        _segments_in_flight.front().header().seqno, _isn, _acked);
...
//! \param[in] ms_since_last_tick the number of milliseconds since the last call to 
//! this method
